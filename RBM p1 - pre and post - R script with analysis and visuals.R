...
library(readxl)
KG.pre <- read_excel("ReframingBehaviorManagementPartOne_4_10_19_Pre_JJ.xls")
KG.post <- read_excel("ReframingBehaviorManagementPartOne_4_10_19_Post_JJ.xls")
...
extractCorrectQ3 <- function(Q3) {
  ...
  if (length(grep("C.What did you learn from that?", Q3)) > 0) {
    return ("1")
  } else {
    return ("0")
  }
...
extractCorrectQ4 <- function(Q4) {
  ...
  if (length(grep("A.feelings", Q4)) > 0) {
    return ("1")
  } else {
    return ("0")
  }
...
write.table(KG.innerjoin.prepost, file="RBMpart1_4.10.19_JJ_PrePost_InnerJoin.csv", sep=",", row.names = FALSE)

write.table(KG.fulljoin.prepost, file="RBMpart1_4.10.19_JJ_PrePost_FullJoin.csv", sep=",", row.names = FALSE)
