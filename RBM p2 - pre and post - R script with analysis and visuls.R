...
library(readxl)
KG.pre <- read_excel("ReframingBehaviorManagementPartTwo_04_24_19_Pre_JJ.xls")
KG.post <- read_excel("ReframingBehaviorManagementPartTwo_04_24_19_Post_JJ.xls")
...
names(KG.pre)[10]<-paste("Q1")
names(KG.pre)[11]<-paste("Q2")
names(KG.pre)[12]<-paste("Q3")
names(KG.pre)[13]<-paste("Q4")
names(KG.pre)[14]<-paste("Q5")
...
extractCorrectQ3 <- function(Q3) {
  ...
  if (length(grep("B.No recess for two days because youth were running in the halls.", Q3)) > 0) {
    return ("1")
  } else {
    return ("0")
  }
...
extractCorrectQ4 <- function(Q4) {
  ...
  if (length(grep("B.determine your feelings about the behavior", Q4)) > 0) {
    return ("1")
  } else {
    return ("0")
  }
...
extractCorrectQ5 <- function(Q5) {
  ...
  if (length(grep("A.Ask the adult to make the other person apologize", Q5)) > 0) {
    return ("1")
  } else {
    return ("0")
  }
...
write.table(KG.innerjoin.prepost, file="RBMpart2_4.24.19_JJ_PrePost_InnerJoin.csv", sep=",", row.names = FALSE)

write.table(KG.fulljoin.prepost, file="RBMpart2_4.24.19_JJ_PrePost_FullJoin.csv", sep=",", row.names = FALSE)
